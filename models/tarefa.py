...
class Tarefa:
    """
    Classe Para representar tarefa, com metodos para salvar, obter,
    excluir e  atualizar tarefas com um banco de dados usando a classe Database.
    """
    ...
    def id(cls, id: int) -> Self:
        with Database() as db:
            query: str = """SELECT titulo_tarefa, data_conclusao, 
        ...
        return cls(
            id_tarefa=id, titulo_tarefa=titulo, data_conclusao=data, 
            concluida=concluida
        )

    def salvar_tarefa(self: Self) -> None:
        with Database() as db:
            query: str = """INSERT INTO tarefas (titulo_tarefa, data_conclusao,
             concluida) VALUES (?, ?, ?);"""
    ...
    def obter_tarefas(cls) -> list[Self]:
        with Database() as db:
            query: str = """SELECT titulo_tarefa, data_conclusao, id,
                concluida FROM tarefas;"""
    ...
    def atualizar_tarefas(self) -> Cursor:
        with Database() as db:
            query: str = """UPDATE tarefas SET titulo_tarefa = ?,
